...
var transCache = angular.module('translatePartialCacheLoader', ['pascalprecht.translate', 'translationCache']);

transCache.provider('$translatePartialCacheLoader', function() {
	...
	Part.prototype.getTable = function(lang, $q, $http, $httpOptions, urlTemplate, errorHandler, cache) {
		var deferred = $q.defer();

		try {
			var cachedData = JSON.parse(cache.get(this.parseUrl(urlTemplate, lang)));
		} catch (e) {

		}
		if (cachedData) {
			this.tables[lang] = cachedData;
	...
	this.$get = ['$rootScope', '$injector', '$q', '$http', function($rootScope, $injector, $q, $http) {
		var service = function(options) {
			...
			var cache = $injector.get('$translationCache');

			angular.forEach(prioritizedParts, function(part) {
				loaders.push(
					part.getTable(options.key, $q, $http, options.$http, options.urlTemplate, errorHandler, cache)
				);
